    ...
    if (document.createElement('canvas').getContext) {  // 判断当前浏览器是否支持绘制海量点
        $.get('${filename}', function (json) {
            json.map(function (point) {
                ...
                var label = new BMap.Label(point.anomalyScore.toFixed(2), {offset: new BMap.Size(20, -10)});
                marker.setLabel(label);
                marker.addEventListener("click", function () {
                    console.log(point.anomalyDisplacement, point.anomalyDistance, point.anomalyTime);
                });
            });
        });
    } else {
